...
const loadData = async () => {
    try {
        ...
        const results = await Promise.allSettled([
            ...
            localforage.getItem(getStorageKey('moments'))
        ]);
        ...
        if (savedMessages && Array.isArray(savedMessages)) {
            ...
            const legacyMoments = messages.filter(m => m.type === 'moment');
            if (legacyMoments.length > 0) {
                moments = [
                    ...legacyMoments.map(m => ({
                        id: m.id || (Date.now() + Math.random()),
                        sender: m.sender || settings.partnerName || '对方',
                        text: m.text || '',
                        timestamp: new Date(m.timestamp),
                        source: 'customReplies'
                    })),
                    ...moments
                ];
                messages = messages.filter(m => m.type !== 'moment');
            ...
            if (backup && Array.isArray(backup.messages) && backup.messages.length > 0) {
                ...
                if (backup.moments && Array.isArray(backup.moments)) {
                    moments = backup.moments.map(m => ({ ...m, timestamp: new Date(m.timestamp) }));
                }
                setTimeout(() => saveData(), 1000);
                showNotification(
        ...
        if (savedMoments && Array.isArray(savedMoments)) {
            moments = savedMoments.map(m => ({ ...m, timestamp: new Date(m.timestamp) }));
        }
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'moments',                val: () => localforage.setItem(getStorageKey('moments'), moments) },
    ...
    if (settings.autoSendEnabled) {
        ...
        const updateUI = () => {
            ...
            const _pillSyncMap = {
                ...
                '#moments-toggle': 'momentsEnabled',
                '#auto-send-toggle': 'autoSendEnabled'
            };
            for (const [sel, prop] of Object.entries(_pillSyncMap)) {
                const el = document.querySelector(sel);
                if (el) {
                    const val = (prop === 'emojiMixEnabled' || prop === 'momentsEnabled') ? (settings[prop] !== false) : !!settings[prop];
    ...
    if (!moments || moments.length === 0) {
        list.innerHTML = `
            <div class="moments-empty">
                ...
                <p>还没有朋友圈动态</p>
                <span>聊天时会按概率从主字卡里抽取</span>
            </div>
        `;
        return;
    }

    const partnerImg = DOMElements.partner && DOMElements.partner.avatar
        ? DOMElements.partner.avatar.querySelector('img')
        : null;
    const avatarHTML = partnerImg && partnerImg.src
        ? `<img src="${escapeMessageHTML(partnerImg.src)}" alt="">`
        : '<i class="fas fa-user"></i>';

    list.innerHTML = moments
        .slice()
        .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
        .map(item => {
            const text = escapeMessageHTML(item.text || '').replace(/\n/g, '<br>');
            const name = escapeMessageHTML(item.sender || settings.partnerName || '对方');
            return `
                <div class="moment-card" data-id="${escapeMessageHTML(item.id)}">
                    <div class="moment-card-header">
                        <div class="moment-avatar">${avatarHTML}</div>
                        <div class="moment-meta">
                            <div class="moment-name">${name}</div>
                            <div class="moment-time">${formatMomentTime(item.timestamp)}</div>
                        </div>
                        <button class="moment-delete" title="删除" data-id="${escapeMessageHTML(item.id)}"><i class="fas fa-times"></i></button>
                    </div>
                    <div class="moment-content">${text}</div>
                    <div class="moment-actions">
                        <span><i class="far fa-heart"></i> 仅你可见</span>
                        <span><i class="far fa-comment-dots"></i> 来自字卡</span>
                    </div>
                </div>
            `;
        }).join('');

    list.querySelectorAll('.moment-delete').forEach(btn => {
        btn.addEventListener('click', () => {
            const id = btn.dataset.id;
            moments = moments.filter(item => String(item.id) !== String(id));
            throttledSaveData();
            renderMoments();
        });
    });
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (msg.type === 'moment') return fragment;
...
const addMessage = (message) => {
    ...
    throttledSaveData();
        ...
        function getAvailableReplyPool() {
            if (!customReplies || customReplies.length === 0) return [];
        ...
        function hideTypingIndicatorNow() {
            try {
            ...
            if (!tiWrapper) return;
            const tiInner = tiWrapper.querySelector('.typing-indicator');
            if (tiInner) {
                tiInner.classList.add('hiding');
                setTimeout(() => {
                    tiWrapper.style.display = 'none';
                    tiInner.classList.remove('hiding');
                }, 240);
            } else {
                tiWrapper.style.display = 'none';
            }
        ...
        window._triggerPartnerMoment = function(options = {}) {
            const replyPool = getAvailableReplyPool();
            if (!replyPool.length) {
                if (!options.silent && typeof showNotification === 'function') {
                    showNotification('朋友圈可用字卡为空，请先添加或启用主字卡', 'info', 3000);
                }
                hideTypingIndicatorNow();
                return false;
            }
            ...
            if (typeof window._sendPartnerNotification === 'function') {
                window._sendPartnerNotification(settings.partnerName || '对方', '发布了一条朋友圈');
            }
            if (!options.silent && typeof showNotification === 'function') {
                showNotification('朋友圈已新增一条动态', 'success', 1800);
            }
            hideTypingIndicatorNow();
            return true;
        };
        ...
        function sendMessage(textOverride = null, type = 'normal') {
            ...
            if (text && text.startsWith('/') && type === 'normal') {
                ...
                if (cmd === '/测试朋友圈' || cmd === '/testmoment') {
                    DOMElements.messageInput.value = '';
                    DOMElements.messageInput.style.height = '46px';
                    if (typeof window._triggerPartnerMoment === 'function') window._triggerPartnerMoment();
                    if (typeof showNotification === 'function') showNotification('✦ 强制触发朋友圈动态', 'info', 1800);
                    return;
                }
            }
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            const momentsChance = Math.max(0, Math.min(1, Number(settings.momentsChance) || 0));
            if (settings.momentsEnabled !== false && momentsChance > 0 && Math.random() < momentsChance) {
                if (typeof window._triggerPartnerMoment === 'function') window._triggerPartnerMoment({ silent: true });
            }
            ...
            const replyPoolOnce = getAvailableReplyPool();
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (importedData && typeof importedData === 'object' &&
                        ...
                        const momentItems = parseVal(getVal('moments'));
                        if (Array.isArray(momentItems)) converted.moments = momentItems;
                    ...
                    const hasMoments   = importedData.moments && Array.isArray(importedData.moments);
                    ...
                    if (!hasMessages && !hasMoments && !hasSettings && !hasReplies && !hasAnn && !hasThemes) {
                    ...
                    overlay.innerHTML = `
                        <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:88%;max-width:360px;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
                            ...
                            <div style="display:flex;flex-direction:column;gap:9px;margin-bottom:20px;">
                                ...
                                ${makeRow('_imp_moments', 'fas fa-images', '朋友圈动态', hasMoments ? `(${importedData.moments.length} 条)` : '', hasMoments, true)}
                    ...
                    if (_impConfirmBtn) _impConfirmBtn.onclick = function() {
                        ...
                        const doMoments  = hasMoments   && !!document.getElementById('_imp_moments')?.checked;
                        ...
                        if (!doMsgs && !doMoments && !doSettings && !doReplies && !doAnn && !doThemes) {
                        ...
                        if (doMoments) {
                            moments = importedData.moments.map(m => ({ ...m, timestamp: new Date(m.timestamp) }));
                        }
                        if (doSettings) {
