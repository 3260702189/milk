...
fileInput.addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        cropImageToSquare(file, 300).then(base64Data => {
            ...
            if (momentsBtn && !momentsBtn.dataset.initialized) {
                momentsBtn.dataset.initialized = 'true';
            ...
            if (closeMomentsBtn && !closeMomentsBtn.dataset.initialized) {
                closeMomentsBtn.dataset.initialized = 'true';
                closeMomentsBtn.addEventListener('click', () => hideModal(document.getElementById('moments-modal')));
            }
            const testMomentsBtn = document.getElementById('moments-test-btn');
            if (testMomentsBtn && !testMomentsBtn.dataset.initialized) {
                testMomentsBtn.dataset.initialized = 'true';
                testMomentsBtn.addEventListener('click', () => {
...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    const toggleSyncMap = {
        ...
        '#moments-toggle': { prop: 'momentsEnabled', name: '朋友圈动态' }
    };
    ...
    updateAutoSendUI();
    updateDelayUI();
    if (typeof updateMomentsChanceUI === 'function') updateMomentsChanceUI();
    ...
    setupAvatarFrameSettings();
            ...
            const momentsChanceSlider = document.getElementById('moments-chance-slider');
            const momentsChanceValue = document.getElementById('moments-chance-value');
            const momentsChanceControl = document.getElementById('moments-chance-control');

            window.updateMomentsChanceUI = function updateMomentsChanceUI() {
                const enabled = settings.momentsEnabled !== false;
                const chance = Math.round(Math.max(0, Math.min(1, Number(settings.momentsChance) || 0)) * 100);
                if (momentsChanceControl) momentsChanceControl.style.display = enabled ? 'block' : 'none';
                if (momentsChanceSlider) momentsChanceSlider.value = chance;
                if (momentsChanceValue) momentsChanceValue.textContent = `${chance}%`;
                const toggle = document.getElementById('moments-toggle');
                if (toggle) toggle.classList.toggle('active', enabled);
            };
            updateMomentsChanceUI();

            if (momentsChanceSlider) {
                momentsChanceSlider.addEventListener('input', (e) => {
                    const chance = parseInt(e.target.value, 10);
                    settings.momentsChance = Math.max(0, Math.min(30, chance)) / 100;
                    updateMomentsChanceUI();
                });
                momentsChanceSlider.addEventListener('change', throttledSaveData);
            }
            ...
            for (const [selector, {
                ...
                const _initVal = (prop === 'emojiMixEnabled' || prop === 'momentsEnabled') ? (settings[prop] !== false) : !!settings[prop];
                element.classList.toggle('active', _initVal);

                element.addEventListener('click', () => {
                    if ((prop === 'emojiMixEnabled' || prop === 'momentsEnabled') && settings[prop] === undefined) settings[prop] = true;
                    settings[prop] = !settings[prop];
                    throttledSaveData();
                    updateUI();
                    element.classList.toggle('active', !!settings[prop]);
                    if (prop === 'momentsEnabled' && typeof updateMomentsChanceUI === 'function') updateMomentsChanceUI();
